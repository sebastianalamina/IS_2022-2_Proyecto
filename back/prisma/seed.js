...
async function seed() {
  ...
  for( let i = 0 ; i < NUMERO_RESTAURANTES-1 ; i++){ 
    ...
    for( let j = 0; j < platillos.length; j++){
    ...
    console.log("Platillos creados", id_platillos); 
    ...
    for(let m=0; m < Math.min(id_clientes.length,idrepartidores.length); m++){
