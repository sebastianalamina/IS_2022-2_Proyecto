...
router.post("/",
  validate(
    ...
    async (req, res) => {
        ...
        try { // <- Issue #45 del repo.
            contenidoCarrito = await prisma.contenidoCarrito.createMany({
       ...
       const { idlciente, tipo, idplatillo} = req.query;
        ...
        try { // <- Issue #45 del repo.
         ...
         if( tipo === "REMOVE"){
            ...
            try { // <- Issue #45 del repo.
                contenidoRemovido = await prisma.contenidoCarrito.deleteMany({
                    where :{
            ...
            try { // <- Issue #45 del repo.
                contenidoAgregado = await prisma.contenidoCarrito.createMany({
...
router.get('/display/:id', async (req, res) => {
    ...
    try { // <- Issue #45 del repo.
        id_carrito = await prisma.modificarCarrito.findFirst({
