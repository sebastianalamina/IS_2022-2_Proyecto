...
router.post(
  ...
  async (req, res) => {
    ...
    try { // <- Issue #45 del repo.
      userCount = await prisma.restaurante.count({
    ...
    let itemCount;
    try { // <- Issue #45 del repo.
      itemCount = await prisma.inventario.count({
        where: {
    ...
    if (itemCount) {
      ...
      let repeatedItem;
      try { // <- Issue #45 del repo.
        repeatedItem = await prisma.inventario.findFirst({
          where: {
            nombre: req.body.nombre,
            idrestaurante: req.body.id_restaurante,
          },
      ...
      let item;
      try { // <- Issue #45 del repo.
        item = await prisma.inventario.update({
          where: {
    ...
    let item;
    try { // <- Issue #45 del repo.
      item = await prisma.inventario.create({
        data: {
          idrestaurante: req.body.id_restaurante,
          cantidad: req.body.cantidad,
          nombre: req.body.nombre,
