...
<template>
  <div v-if="!this.restQueried">
    <form @submit="buscarRestauranteBoton">
      ...
      <va-button type="submit" value="Buscar" />
    </form>
  </div>
  ...
  <div v-else-if="!this.tableQueried">
    <form @submit="buscarMesaBoton">
      <label>
        <h1>Introduce el número de la mesa:</h1>
      </label>
  ...
  <div v-else>
    <h1>
      Viendo la mesa #{{ this.idMesa }} del restaurante #{{
        this.idRestaurante
      }}...
    </h1>
    ...
    <div v-if="this.orden == null || this.orden.length == 0">
      ...
      <form @submit="iniciarOrden">
        <va-button type="submit" value="Crear orden">Crear orden</va-button>
      </form>
    </div>
    <!--Fin del caso "no hay orden"-->

    <div v-else>
      <h6 v-if="this.orden.length > 1">
        ¡Hay más de una orden abierta para esta mesa!
      </h6>
      La mesa está {{ this.mesa.ocupada ? "ocupada" : "libre" }}
      <form class="mismalinea" @submit="cambiarOcupada">
        <va-button type="submit" value="Cambiar">Cambiar</va-button>
      </form>
      <br />
      Órdenes abiertas para esta mesa: {{ this.orden.length }} <br /><br />
      <div class="mismalinea" v-for="ord in orden">
        ...
        <select v-model="this.selected" multiple>
          <option
            v-for="item in ord.contenido"
            v-bind:value="{
              idcontenidoorden: item.idcontenidoorden,
              costo: item.platillo.costo,
              nombre: item.platillo.nombre,
            }"
          >
            ID:{{ item.idcontenidoorden }} {{ item.platillo.nombre }} ${{
              item.platillo.costo
            }}
          </option>
        </select>
      ...
      <div v-if="this.mostrarBotones">
        <form class="mismalinea" @submit="anadirOrden">
          <va-button type="submit" value="Añadir orden">Añadir orden</va-button>
        </form>
        <form class="mismalinea" @submit="agregarPlatillo">
          <va-button type="submit" value="Agregar platillo"
            >Agregar platillo</va-button
          >
        </form>
        <form class="mismalinea" @submit="eliminarSeleccion">
          <va-button type="submit" value="Eliminar selección"
            >Eliminar selección</va-button
          >
        </form>
        <form class="mismalinea" @submit="cerrarOrden">
          <va-button type="submit" value="Cerrar orden">Cerrar orden</va-button>
        </form>
      ...
      <div v-if="this.mostrarAgregar">
        <form @submit="agregarPlatillo">
          ...
          <select v-model="this.platilloParaAgregar">
            <option
              v-for="plat in this.platillosDisponibles"
              v-bind:value="plat.idplatillo"
            >
              {{ plat.nombre }}
            </option>
          </select>
          <br />
          <va-button type="submit" value="Agregar">Agregar</va-button>
        </form>
        <form @submit="cancelar">
          <va-button type="submit" value="Cancelar">Cancelar</va-button>
        </form>
      </div>
      ...
      <div v-if="this.mostrarEliminar">
        ...
        <ul>
          <li v-for="elemento in this.selected">
            ID:{{ elemento.idcontenidoorden }} {{ elemento.nombre }} ${{
              elemento.costo
            }}
          </li>
        </ul>
        ...
        <form @submit="eliminarSeleccion">
          <va-button type="submit" value="Confirmar">Confirmar</va-button>
        </form>
        <form @submit="cancelar">
          <va-button type="submit" value="Cancelar">Cancelar</va-button>
        </form>
      ...
      <div v-if="this.mostrarCerrar">
        <form @submit="cerrarOrden">
          ...
          <select v-model="this.ordenPorCerrar">
            <option
              v-for="ord in this.orden"
              v-bind:value="{ idorden: ord.idorden, costo: ord.costo }"
            >
              {{ ord.idorden }}
            </option>
          </select>
          <br />
          Costo total:
          <span v-if="this.ordenPorCerrar != null">{{
            this.ordenPorCerrar.costo
          }}</span>
          <br />
          <va-button type="submit" value="Confirmar">Confirmar</va-button>
        </form>
    ...
    <!--Fin del caso "sí hay orden"-->
  </div>
  <!--Fin del caso "consultados restaurante y mesa"-->
